...
export function SocialProof() {
  return (
    <section id="case-study" className="bg-[color:var(--surface-secondary)] py-20 md:py-24 scroll-mt-24">
      <Container>
        <SectionTitle>What teams measure after switching.</SectionTitle>

        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {metrics.map(({ stat, label, source }) => (
            <article key={label.slice(0, 20)} className="card p-6">
              <p className="text-2xl font-medium tracking-tight text-[color:var(--foreground)]">{stat}</p>
              <p className="mt-2 text-[13px] leading-relaxed text-[color:var(--muted)]">{label}</p>
              <p className="mt-4 text-[11px] text-[color:var(--muted)]">{source}</p>
            </article>
          ))}
        </div>

        <blockquote className="card mx-auto mt-12 max-w-[68ch] p-8 md:p-10">
